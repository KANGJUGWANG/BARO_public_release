        ...
        try:
            ...
            for (origin, dest), (ow_url, ow_cards) in zip(ONEWAY_ROUTES, oneway_results):
                ...
                (output_dir / file_name).write_text(
                    json.dumps({"observed_at": collected_at, "route_type": "oneway", "origin": origin, "dest": dest, "dep_date": dep_date.isoformat(), "ret_date": None, "stay_nights": None, "dpd": dpd, "search_url": ow_url, "card_count": len(ow_cards), "cards": ow_cards}, ensure_ascii=False, indent=2),
                    encoding="utf-8",
                )
            ...
            for (origin, dest), (rt_url, rt_combos) in zip(ROUNDTRIP_ROUTES, roundtrip_results):
                ...
                (output_dir / file_name).write_text(
                    json.dumps({"observed_at": collected_at, "route_type": "roundtrip", "origin": origin, "dest": dest, "dep_date": dep_date.isoformat(), "ret_date": ret_date.isoformat(), "stay_nights": STAY_NIGHTS, "dpd": dpd, "search_url": rt_url, "combo_count": len(rt_combos), "combos": rt_combos}, ensure_ascii=False, indent=2),
                    encoding="utf-8",
                )

            log.info("[DPD=%s %s] \uc644\ub8cc  \ud3b8\ub3c4=%s\uac74  \uc655\ubcf5=%s\uac74", dpd, dep_date, sum(len(c) for _, c in oneway_results), sum(len(c) for _, c in roundtrip_results))
            return True
        except Exception as e:
            log.warning("[DPD=%s %s] \uc2e4\ud328 (%s): %s", (dep_date - date.today()).days, dep_date, type(e).__name__, e)
            return False


async def _collect_batch(browser, dep_dates, sem, collected_at, output_dir):
    results = await asyncio.gather(*[collect_date(browser, d, sem, collected_at, output_dir) for d in dep_dates])
...
async def run_collection(dep_date: Optional[date] = None) -> None:
    ...
    deadline = datetime.now() + timedelta(minutes=CYCLE_INTERVAL_MIN - RETRY_DEADLINE_MARGIN_MIN)

    if dep_date is not None:
        dep_dates = [dep_date]
        log.info("=== \uc218\uc9d1 \uc2dc\uc791 (\ub2e8\uc77c \ub0a0\uc9dc): %s ===", dep_date.isoformat())
    else:
        dep_dates = [today + timedelta(days=dpd) for dpd in range(DPD_MIN, DPD_MAX + 1)]
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            while failed and datetime.now() < deadline:
                ...
                log.info("\uc7ac\uc2dc\ub3c4 %s\ud68c: \uc2e4\ud328 %s\uac1c, %.0f\ucd08 \ub300\uae30", retry_round, len(failed), wait_sec)
                await asyncio.sleep(wait_sec)
                if datetime.now() >= deadline:
                    break
                still_failed = await _collect_batch(browser, failed, sem, collected_at, output_dir)
                log.info("\uc7ac\uc2dc\ub3c4 %s\ud68c \uc644\ub8cc: \ubcf5\uad6c %s\uac1c, \uc5ec\uc804 \uc2e4\ud328 %s\uac1c", retry_round, len(failed) - len(still_failed), len(still_failed))
                failed = still_failed
                retry_round += 1

            if failed:
                log.warning("\ucd5c\uc885 \uc2e4\ud328 DPD (%s\uac1c): %s", len(failed), sorted((d - today).days for d in failed))
            else:
