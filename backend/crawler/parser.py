...
def _is_hm(value) -> bool:
    return (isinstance(value, list) and len(value) == 2 and isinstance(value[0], int) and isinstance(value[1], int) and 0 <= value[0] <= 23 and 0 <= value[1] <= 59)
            ...
            if len(fi) > 24 and isinstance(fi[24], list):
                for seller in fi[24]:
                    if isinstance(seller, list) and seller and seller[0] == airline_code:
                        official_seller = {"code": seller[0], "name": seller[1] if len(seller) > 1 else None, "url": seller[2] if len(seller) > 2 else None}
                        break
        except Exception:
            pass
